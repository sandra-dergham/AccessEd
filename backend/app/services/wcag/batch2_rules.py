...
# Shared helpers (used by 2.4.1 and 2.4.5 — defined once to avoid duplication)
...
def _tree_has_heading(nodes: list) -> bool:
    ...
    for node in nodes:
        ...
        if node.get("role") in heading_roles:
            return True
        real_children = [c for c in node.get("children", []) if isinstance(c, dict)]
        if _tree_has_heading(real_children):
            return True
    return False
...
def check_info_relationships(document_model: dict) -> list[dict]:
    ...
    Flags the document when structural information is missing or too weak
    to make relationships (headings, lists, tables) programmatically
    determinable.
    """
    ...
    def tree_has_meaningful_role(nodes: list) -> bool:
        for node in nodes:
            if not isinstance(node, dict):
                continue
            if node.get("role") in meaningful_roles:
                return True
            real_children = [c for c in node.get("children", []) if isinstance(c, dict)]
...
def check_meaningful_sequence(document_model: dict) -> list[dict]:
    ...
    Checks whether a reading order exists and avoids obvious sequencing
    problems (empty order, duplicates, page regressions, y-coordinate jumps).

    Note: invalid-ID and missing-block checks are intentionally omitted —
    the parser builds reading_order and text_blocks in lockstep so those
    checks can never fire and would only produce false negatives.
    """
    doc           = document_model.get("document", {})
    ...
    # Guard: duplicate block IDs in the order
    if len(reading_order) != len(set(reading_order)):
        issues.append(
            make_issue(
                ...
                location={"scope": "document"},
                severity="medium",
                recommendation="Ensure the reading order does not repeat text blocks.",
            )
        )
        return issues

    # Spatial checks: page regression and y-coordinate regression
    block_map     = {b.get("id"): b for b in text_blocks if b.get("id")}
    previous_page = None
    previous_y    = None
...
def check_sensory_characteristics(document_model: dict) -> list[dict]:
    ...
    Heuristic check for instructional text that relies solely on color,
    shape, or visual position.  Requires an instruction keyword and a
    sensory keyword to appear within a 6-word window of each other to
    reduce false positives on ordinary prose.
    """
    doc        = document_model.get("document", {})
    ...
    multi_word_sensory = ["next to"]

    def keywords_are_proximate(text: str, window: int = 6) -> bool:
        words = text.lower().split()

        # Single-word proximity check
        for i, word in enumerate(words):
            if word in instruction_keywords:
                surrounding = words[max(0, i - window): i + window + 1]
                if any(s in surrounding for s in single_sensory_keywords):
                    return True

        # Multi-word sensory phrase proximity check
        lowered = text.lower()
        for phrase in multi_word_sensory:
            if phrase in lowered:
                phrase_pos  = lowered.find(phrase)
                nearby_text = lowered[max(0, phrase_pos - 40): phrase_pos + 40]
                if any(kw in nearby_text for kw in instruction_keywords):
                    return True
    ...
    for span in text_spans:
        ...
        if keywords_are_proximate(text):
            issues.append(
                make_issue(
                    criterion="1.3.3",
                    ...
                    severity="needs_review",  # fixed: was "needs review" (space)
                    recommendation=(
...
def check_identify_input_purpose(document_model: dict) -> list[dict]:
    ...
    Flags form fields whose purpose is not programmatically identifiable
    because the field name is missing, too short, or generic.
    """
    doc         = document_model.get("document", {})
    ...
    generic_names = {
        "field", "input", "textbox", "text", "box",
        "form", "untitled", "unknown", "default",
    }
...
def check_images_of_text(document_model: dict) -> list[dict]:
    ...
    Emits a pass when images exist but none contain meaningful text, so the
    criterion always appears in the report.
    """
    doc               = document_model.get("document", {})
    image_occurrences = doc.get("images", {}).get("occurrences", [])
    issues: list[dict] = []

    # No images — criterion not applicable
    if not image_occurrences:
        issues.append(
            make_issue(
                ...
                recommendation="No images found in this document.",
            )
        )
        return issues
    ...
    # Images exist but none contained meaningful text — explicit pass
    if not flagged_any:
        issues.append(
            make_issue(
                criterion="1.4.5",
                issue="no_images_of_text_detected",
                location={"scope": "document"},
                severity="pass",
                recommendation="No images containing meaningful text were detected.",
            )
        )
...
def check_reflow(document_model: dict) -> list[dict]:
    ...
    Heuristic check for wide text blocks that may indicate fixed-layout
    content requiring horizontal scrolling in narrow viewports.
    """
    doc         = document_model.get("document", {})
    pages       = doc.get("pages", [])
    text_blocks = doc.get("text_blocks", [])
...
def check_text_spacing(document_model: dict) -> list[dict]:
    ...
    for span in text_spans:
        spans_by_page.setdefault(span.get("page_index"), []).append(span)
...
def check_bypass_blocks(document_model: dict) -> list[dict]:
    ...
    PDF-adapted heuristic: checks whether longer documents provide a
    navigation mechanism (bookmarks, internal links, or heading structure)
    so users can bypass repeated or non-essential content.
    Only evaluated on documents with 3+ pages.
    """
    doc   = document_model.get("document", {})
    pages = doc.get("pages", [])
    issues: list[dict] = []
    ...
    has_bookmarks          = bool(doc.get("bookmarks", []))
    has_internal_links     = any(_is_internal_link(l) for l in doc.get("links", []))
    has_heading_navigation = _tree_has_heading(doc.get("structure", {}).get("tree") or [])
...
def check_page_titled(document_model: dict) -> list[dict]:
    ...
    generic_titles = {
        "untitled", "document", "pdf", "file",
        "scan", "scanned document", "default", "test",
    }
    ...
    if is_missing or is_too_short or is_generic:
        issues.append(
            make_issue(
                ...
                location={"scope": "document"},
                severity="medium",
                recommendation="Provide a clear, descriptive document title in the PDF metadata.",
            )
        )
...
def check_link_purpose(document_model: dict) -> list[dict]:
    ...
    vague_texts = {
        "click here", "here", "more", "read more", "more info",
        "learn more", "details", "link", "this", "continue",
    }
        ...
        if is_missing or is_vague or is_too_short or is_raw_url:
            ...
            issues.append(
                make_issue(
                    ...
                    severity="medium",
                    recommendation=(
                        "Provide descriptive link text that clearly explains "
...
def check_multiple_ways(document_model: dict) -> list[dict]:
    ...
    PDF-adapted heuristic: checks whether longer documents provide at least
    two distinct navigation methods (bookmarks, internal links, heading
    structure).  Only evaluated on documents with 3+ pages.
    """
    doc   = document_model.get("document", {})
    pages = doc.get("pages", [])
    issues: list[dict] = []
    ...
    navigation_methods = sum([has_bookmarks, has_internal_links, has_heading_navigation])
...
def check_headings_labels(document_model: dict) -> list[dict]:
    ...
    structure_tree     = structure.get("tree") or []
    has_heading_tags   = _tree_has_heading(structure_tree)
...
def check_language_of_page(document_model: dict) -> list[dict]:
    ...
    has_valid_declared_lang = (
        bool(normalized_lang) and re.match(lang_pattern, normalized_lang) is not None
    )
...
def check_language_of_parts(document_model: dict) -> list[dict]:
    ...
    # Resolve document language — prefer content-inferred over embedded /Lang
    if inferred_lang:
        ...
        return []  # no language baseline — nothing to compare against

    issues: list[dict] = []  # single declaration
    MIN_TEXT_LENGTH = 20
    ...
    for span in text_spans:
        ...
        if any(marker in lowered_text for marker in ("http://", "https://", "www.", "@")):
            continue
        ...
        if set(lowered_text.split()).intersection(LANGUAGE_NAME_PATTERNS):
            continue
